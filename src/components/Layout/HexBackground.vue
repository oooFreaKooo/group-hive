<template>
    <div class="bg">
        <svg
            ...
            class="position-absolute top-50 start-50 translate-middle w-100 vh-100"
        >
            <defs>
                <radialGradient
                    id="polygonGradient"
                ...
                >
                    <stop
                        offset="10%"
                        stop-color="var(--bs-tertiary)"
                    />
                    <stop
                        offset="100%"
                        stop-color="var(--bs-light)"
                    />
                </radialGradient>
            </defs>
            ...
            >
                ...
                <polygon points="937.4 397.3 937.4 359.3 970.4 340.2 1003.3 359.3 1003.3 397.3 970.4 416.3 937.4 397.3" />
...
const hexsRef = ref<SVGGElement>()
const startTextRef = ref<SVGTextElement>()
const redDotRef = ref<SVGCircleElement>()

// GSAP timeline
let timeline: gsap.core.Timeline | null = null

// State
const polygonsRows: SVGPolygonElement[][] = []
let centerPolygon: SVGPolygonElement | null = null

// Initialize polygons grid
const initPolygonsGrid = () => {
    if (!hexsRef.value) { return }

    const polygons = hexsRef.value.querySelectorAll('polygon')
    const center = 85
    centerPolygon = polygons[center]

    if (centerPolygon) {
        centerPolygon.classList.add('center')
    }
...
const updateCenterText = () => {
    if (!startTextRef.value || !centerPolygon) { return }

    const bbox = centerPolygon.getBBox()
    startTextRef.value.setAttribute('x', bbox.x.toString())
    startTextRef.value.setAttribute('y', bbox.y.toString())
    startTextRef.value.setAttribute('textLength', (bbox.width - 10).toString())
}

// Animation setup
const setupAnimation = () => {
    if (!hexsRef.value || !startTextRef.value || !redDotRef.value) { return }

    const polygons = hexsRef.value.querySelectorAll('polygon')
    const center = 85

    timeline = gsap.timeline({ delay: 0.5, paused: true })
        .addLabel('start')
        .set(polygons, { opacity: 0, scale: 0 })
        .set(startTextRef.value, { opacity: 0, scale: 0, y: '50%' })
        .set(redDotRef.value, { visibility: 'hidden' })
        .to(polygons, {
            duration: 1,
            opacity: 1,
            scale: 1,
        ...
        // Hide red dot and begin hexagon sequence
        .set(redDotRef.value, { visibility: 'hidden' })
        .set(polygons[center], { opacity: 1 })
        // Fade in surrounding hexagons in sequence
        .to(polygons[center - 19], { duration: 0.1, opacity: 0.2, delay: 0.1 })
        .to(polygons[center - 20], { duration: 0.1, opacity: 0.2, delay: 0.1 })
        .to(polygons[center - 1], { duration: 0.1, opacity: 0.2, delay: 0.1 })
        .to(polygons[center + 18], { duration: 0.1, opacity: 0.2, delay: 0.1 })
        .to(polygons[center + 19], { duration: 0.1, opacity: 0.2, delay: 0.1 })
        .to(polygons[center + 1], { duration: 0.1, opacity: 0.2, delay: 0.1 })
        // Fade out center hexagon
        .to(polygons[center], { duration: 0.1, opacity: 0, delay: 0.1 })
        // Scale up start text with elastic effect

        .addLabel('Welcome!')
        .call(() => {
            if (startTextRef.value) { startTextRef.value.textContent = 'Welcome!' }
        })
        .to(startTextRef.value, {
            duration: 0.5,
            scale: 2.2,
            transformOrigin: '50% 50%',
            ease: 'elastic.out(1, 0.3)',
        }, '-=0.6')
        .set(startTextRef.value, {
            opacity: 1,
            attr: { textLength: 160 },
            scale: 1,
            x: -55,
        ...
        .to(startTextRef.value, { duration: 0.7, opacity: 1 }, 'Welcome!')

        .call(() => {
            if (startTextRef.value) { startTextRef.value.textContent = 'to' }
        })
        .to(startTextRef.value, {
            duration: 1,
            attr: { textLength: 50 },
        ...
        .call(() => {
            if (startTextRef.value) { startTextRef.value.textContent = 'GroupHive' }
        })
        .to(startTextRef.value, {
            duration: 0.5,
            attr: { textLength: 170 },
        ...
        // Fade out all hexagons with elastic effect
        .to(polygons, {
            duration: 1,
            opacity: 0,
        ...
        .to(startTextRef.value, { duration: 0.2, opacity: 0 }, '-=1.2')
        .to(polygons, {
            duration: 1,
            opacity: 1,
            scale: 1,
...
onMounted(() => {
    ...
    setupAnimation()
    playAnimation()
    window.addEventListener('resize', updateCenterText)
})

onUnmounted(() => {
    window.removeEventListener('resize', updateCenterText)
    if (timeline) {
        timeline.kill()
    }
})
...
.bg {
    ...
    width: 100%;
    height: 100vh;
    background: linear-gradient(to right, var(--bs-dark-subtle), var(--bs-dark), var(--bs-light));
